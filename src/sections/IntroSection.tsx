    ...
    <section className="container mx-auto h-screen w-full max-w-5xl px-4 sm:px-8 font-sans pb-10 md:pb-20">
      <div className="grid h-full grid-cols-2 items-center justify-center text-center md:text-start">
        ...
        <div className="order-2 col-span-2 space-y-5 md:order-1 md:col-span-1">
          ...
          {content.cta && (
            <div className="cta flex justify-start py-4">
              <a
        ...
        <div className="order-1 col-span-2 flex flex-col items-center justify-center md:order-2 md:col-span-1">
          <div className="flex max-w-xs flex-col items-center justify-center rounded-xl py-10 dark:bg-gray-900 dark:text-gray-100 md:p-6 md:px-12 ">
            <div className="w-36 rounded-full bg-white p-2 md:w-full">
              <Image
                src={content.profilePhoto}
                alt={`${content.name}'s Profile Photo`}
            ...
            <div className="space-y-4 divide-y divide-gray-700 text-center">
              <div className="my-2 space-y-1">
                ...
                <p className="px-5 text-sm font-bold text-purple-500">
                  {content.title}
                </p>
              </div>
              <div className="align-center flex justify-center space-x-4 pt-2">
                {content.socialMedia.github && (
                  <GitHubLink url={content.socialMedia.github} />
                )}
                {content.socialMedia.dribbble && (
                  <DribbbleLink url={content.socialMedia.dribbble} />
                )}
                {content.socialMedia.twitter && (
                  <TwitterLink url={content.socialMedia.twitter} />
                )}
                {content.socialMedia.email && (
                  <EmailLink email={content.socialMedia.email} />
                )}
              </div>
            </div>
          </div>
...
const GitHubLink = ({ url }: { url: string }) => (
  <a
    ...
    href={url}
    aria-label="GitHub"
    className="rounded-md p-2 dark:text-gray-100 hover:dark:text-violet-400"
  >
    <svg
      viewBox="0 0 496 512"
...
const DribbbleLink = ({ url }: { url: string }) => (
  <a
    rel="noopener noreferrer"
    href={url}
    aria-label="Dribble"
